...
@section('container-fluid')
    <div class="container pt-2">
        <div class="row">
            <div class="col-12">
                <table class="table table-dark table-striped">
                    ...
                    <tbody>
                        @foreach (Session::get("matkul") as $key => $item)
        ...
        <div class="row">
            <div class="col-2">
                <button class="btn btn-primary" id="modal-btn">Add Matkul</button>
            </div>
        </div>
    </div>
@endsection
...
<form action="/master/matkul/insert" method="post" id="form-insert">
    @csrf
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title">Insert Matkul</h5>
...
@section('custom-js')
    <script>
        $("#delete-form").attr("action", "/master/matkul/delete");
        ...
        $(document).on("click", ".btn-close-modal", function(){
            $("#modal-insert").hide();
            $('#form-insert')[0].reset();
            $('#form-insert').attr("action", "/master/matkul/insert");
            $('#form-insert .modal-title').text("Insert Matkul");
        ...
        $(document).on("click", ".btn-edit", function(){
            $("#modal-insert").show();
            $("#form-insert").attr("action", "/master/matkul/edit");
            $("#form-insert").find(".modal-title").text("Edit Matkul");

            // edit index
            var editIdx = parseInt($(this).parent("td").attr("index"));
            $("#edit-index").val(editIdx);

            let matkul = @json(Session::get("matkul"))[editIdx];
            $("#jurusan").val(matkul.jurusan_uid);
            $("#matkul").val(matkul.matkul_nama);
        });
    </script>
@endsection